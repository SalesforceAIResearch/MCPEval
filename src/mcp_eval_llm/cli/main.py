...
class ColoredHelpFormatter(argparse.HelpFormatter):
    ...
    def _format_action(self, action):
        ...
        sorted_commands = sorted(self.command_colors.items(), key=lambda x: len(x[0]), reverse=True)
...
def task_generator(args):
    ...
    server_paths, server_args_list = parse_servers_argument(args)
    
    if len(server_paths) > 1:
        colored_print(f"✨ Connecting to {len(server_paths)} servers for multi-server task generation", Colors.BRIGHT_GREEN)
        # Set multi-server attributes
        args.server_paths = server_paths
        args.server_args_list = server_args_list
    else:
        colored_print(f"🔗 Connecting to single server: {server_paths[0]}", Colors.BRIGHT_BLUE)
    ...
    from mcp_eval_llm.cli.mcp_task_generator.generate import main
    main(args)
...
def task_verifier(args):
    ...
    if len(server_paths) > 1:
        colored_print(f"✨ Connecting to {len(server_paths)} servers for multi-server task verification", Colors.BRIGHT_GREEN)
        # Set multi-server attributes
        args.server_paths = server_paths
        args.server_args_list = server_args_list
    else:
    ...
    from mcp_eval_llm.cli.mcp_task_verifier.verify import main
    main(args)


def data_converter(args):
    """Entry point for data format conversion subcommand."""
    from mcp_eval_llm.utils.task_converter import process_jsonl_file, process_file
    import json
    import os
    ...
    if args.prompt_file:
        try:
            with open(args.prompt_file, 'r') as f:
                ...
                colored_print(f"✓ Loaded system message from {args.prompt_file}", Colors.GREEN)
        except Exception as e:
            colored_print(f"⚠️  Warning: Could not load prompt file {args.prompt_file}: {e}", Colors.YELLOW)
    ...
    if args.input.endswith('.jsonl'):
        ...
        process_jsonl_file(args.input, args.output, args.prefix, args.split, 
                          system_message=system_message)
    else:
        # Process a single JSON file
        process_file(args.input, args.output, args.task_id, args.prefix, args.split, 
                    system_message=system_message)
    
    colored_print(f"✓ Data format conversion completed. Output saved to {args.output}", Colors.BRIGHT_GREEN, bold=True)


def data_splitter(args):
    """Entry point for data splitting subcommand."""
    from mcp_eval_llm.utils.data_splitter import split_jsonl_file
...
def model_evaluator(args):
    ...
    if len(server_paths) > 1:
        colored_print(f"✨ Connecting to {len(server_paths)} servers for multi-server model evaluation", Colors.BRIGHT_GREEN)
        # Set multi-server attributes
        args.server_paths = server_paths
        args.server_args_list = server_args_list
    else:
    ...
    from mcp_eval_llm.cli.mcp_model_evaluator.evaluate import main
    main(args)


def analyzer(args):
    """Entry point for analysis subcommand."""
    from mcp_eval_llm.cli.analyzer.analyze import main
    main(args)


def llm_judger(args):
    """Entry point for LLM judger subcommand."""
    from mcp_eval_llm.cli.llm_judger.judge import main
    main(args)


def llm_judger_analyzer(args):
    """Entry point for LLM judger analyzer subcommand."""
    from mcp_eval_llm.cli.llm_judger.analyze import main
    main(args)


def auto_workflow(args):
    """Entry point for auto workflow subcommand."""
    from mcp_eval_llm.cli.auto.auto import main
    main(args)
...
def parse_arguments():
    ...
    if sys.stdout.isatty():
        description = (f"{Colors.BRIGHT_CYAN}🔧 MCPEval CLI tools for {Colors.BRIGHT_WHITE}"
    ...
    task_gen_parser = subparsers.add_parser(
        'generate-tasks',
        help='Generate tasks for MCP servers',
    ...
    task_gen_parser.add_argument(
        ...
        type=str,
        nargs='+',
        help='Server paths to connect to. Format: server_path[:args] where args is comma-separated. '
             'Examples: @modelcontextprotocol/server-fetch, '
             '@openbnb/mcp-server-airbnb:--ignore-robots-txt, '
             'server1 server2:--flag1,--flag2'
    )
    # Keep --server for backward compatibility (deprecated)
    task_gen_parser.add_argument(
        '--server',
        type=str,
        help='[DEPRECATED] Use --servers instead. MCP server script path'
    )
    task_gen_parser.add_argument(
        '--server-args',
        type=str,
        nargs='*',
        default=[],
        help='[DEPRECATED] Use --servers with server:args syntax instead. Additional arguments to pass to the MCP server'
    )
    ...
    task_conv_parser.add_argument(
        ...
        type=str,
        required=True,
        help='Output file path for converted data'
    )
    task_conv_parser.add_argument(
        '--prefix',
        type=str,
    ...
    split_parser = subparsers.add_parser(
        'split-data',
        help='Randomly split JSONL task file into train/valid/test sets'
    )
    split_parser.add_argument(
        '--input',
        type=str,
        required=True,
        help='Input JSONL file path to split'
    )
    split_parser.add_argument(
        '--output-dir',
        type=str,
    ...
    eval_parser.add_argument(
        ...
        type=int,
        default=30,
        help='Maximum number of turns for task execution (default: 3)'
    )
    eval_parser.add_argument(
        '--force-rerun',
        action='store_true',
    ...
    analyze_parser = subparsers.add_parser(
        'analyze',
        help='Analyze evaluation results against ground truth'
    )
    ...
    analyze_parser.add_argument(
        ...
        type=str,
        default=None,
        help='Generate report directly from an existing summary analysis JSON file'
    )
    ...
    llm_judge_parser.add_argument(
        '--input-file',
        type=str,
        help='Path to JSON evaluation file to judge'
    )
    ...
    llm_judge_analyze_parser.add_argument(
        ...
        type=str,
        required=True,
        help='Path to trajectory scores JSON file'
    )
    ...
    llm_judge_analyze_parser.add_argument(
        ...
        type=str,
        required=True,
        help='Path to completion scores JSON file'
    )
...
def main():
    ...
    try:
        ...
        # Success message
        colored_print(f"✅ Command '{args.command}' completed successfully!", Colors.BRIGHT_GREEN, bold=True)

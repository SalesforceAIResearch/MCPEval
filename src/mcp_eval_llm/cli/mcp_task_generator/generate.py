...
from mcp_eval_llm.commons.types import Task, ToolLibrary
from mcp_eval_llm.utils.cli import print_task, setup_colored_logging
...
async def generate_tasks(args):
    ...
    try:
        ...
        if hasattr(args, 'server_paths') and hasattr(args, 'server_args_list'):
            ...
            logger.info(f"Connecting to {len(server_paths)} MCP servers")
            ...
            await client.connect_to_server(args.server, args.server_args)
        ...
        while tasks_generated < num_tasks:
            try:
                ...
                logger.info(f"Starting generation for task {tasks_generated + 1}/{num_tasks}")
                ...
                tasks_generated += 1
                logger.info(f"Generated task {tasks_generated}/{num_tasks}: {task.name}")
        ...
        logger.info(f"Task generation complete. {tasks_generated} tasks saved to {args.output}")
...
if __name__ == "__main__":
    ...
    print("Please use the main CLI: python -m mcp_eval_llm.cli.main generate-tasks --help")
    import sys
    sys.exit(1) 
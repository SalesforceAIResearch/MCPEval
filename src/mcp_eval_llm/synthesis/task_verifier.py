...
Goal: {task.goal}"""
        ...
        for turn in range(max_turns):
            ...
            if assistant_message.get("tool_calls"):
                logger.debug(f"Tool calls: {[tc['function']['name'] for tc in assistant_message.get('tool_calls', [])]}")
            ...
            for tool_call in tool_calls:
                if tool_call["type"] == "function" and tool_call["function"]["name"] == "request_task_updating":
                    try:
                        import json
                ...
                if tool_call["type"] == "function":
                    ...
                    if session:
                        try:
                            ...
                            tool_call_result = ToolCallResult(content=content, is_error=False)
                            all_tool_call_results.append((tool_call_obj, tool_call_result))
                            ...
                            tool_call_result = ToolCallResult(content=error_message, is_error=True)
                            all_tool_call_results.append((tool_call_obj, tool_call_result))
                            messages.append({
                                "role": "tool",
                                "tool_call_id": tool_call.get("id", ""),

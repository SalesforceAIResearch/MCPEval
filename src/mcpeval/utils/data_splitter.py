...
def validate_ratios(train_ratio: float, valid_ratio: float, test_ratio: float) -> None:
    ...
    if abs(total_ratio - 1.0) > 1e-6:
        raise ValueError(
            f"Train, valid, and test ratios must sum to 1.0, got {total_ratio}"
        )
        ...
        for i, line in enumerate(lines):
            ...
            if line:
                try:
                    ...
                    colored_print(
                        f"⚠️  Warning: Skipping invalid JSON at line {i+1}: {e}",
                        Colors.YELLOW,
                    )
...
def split_tasks(
    ...
    colored_print(
        f"✓ Split into {len(train_tasks)} train, {len(valid_tasks)} valid, {len(test_tasks)} test tasks",
        Colors.GREEN,
    )
...
def generate_output_paths(
    input_file: str, output_dir: str = None
) -> Tuple[str, str, str]:
...
def create_split_summary(
    ...
    summary = {
        ...
        "seed": seed,
        "output_files": {"train": train_path, "valid": valid_path, "test": test_path},
    }
...
def split_jsonl_file(
    ...
    # Split tasks
    train_tasks, valid_tasks, test_tasks = split_tasks(
        tasks, train_ratio, valid_ratio, test_ratio, seed
    )
    ...
    # Print summary
    colored_print(
        f"✓ Data splitting completed successfully!", Colors.BRIGHT_GREEN, bold=True
    )

        ...
        for tool in tools:
            try:
                ...
                if hasattr(tool, "to_openai_function_schema"):
                    formatted_tool = getattr(tool, "to_openai_function_schema")()
                    formatted_tools.append(formatted_tool)
                    logger.debug(f"Converted tool: {tool.name}")
                else:
                    logger.warning(
                        f"Tool {getattr(tool, 'name', 'unknown')} does not have to_openai_function_schema method"
                    )
        ...
        **llm_kwargs,
        ...
        if messages is None:
            ...
            messages = [
                ...
                {
                    "role": "user",
                    "content": f"Please complete the following task: {task.name}. {task.description}.",
                },
            ]
        ...
        try:
            ...
            for turn in range(max_turns):
                ...
                try:
                    ...
                    logger.debug(
                        f"Assistant response: {assistant_message.get('content')}"
                    )
                    if assistant_message.get("tool_calls"):
                        ...
                        if (
                            tool_calls
                            and hasattr(tool_calls[0], "function")
                            ...
                            logger.debug(
                                f"Tool calls present but in unknown format: {type(tool_calls[0]).__name__ if tool_calls else 'none'}"
                            )
                    ...
                    # Handle error in model inference
                    error_message = (
                        f"Error during model inference on turn {turn+1}: {str(e)}"
                    )
                ...
                for tool_call in tool_calls:
                    try:
                        ...
                        if (
                            ...
                            and getattr(tool_call, "type") == "function"
                            ...
                            logger.warning(
                                f"Unsupported tool call format: {type(tool_call).__name__}"
                            )
                            continue
                        ...
                        if session:
                            ...
                            # No session available for this tool
                            error_message = f"No session available for tool {tool_name}"
                        ...
                        # Handle any other error in tool call processing
            ...
            # Handle any unexpected exception in the entire process
            error_message = f"Unexpected error during task execution: {str(e)}"
            logger.error(error_message, exc_info=True)

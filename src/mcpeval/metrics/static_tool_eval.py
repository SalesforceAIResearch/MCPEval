...
class ToolEvalResult(BaseModel):
    ...
    task_id: str = Field(
        "unknown", description="Identifier of the task being evaluated"
    )
    success: bool = Field(False, description="Whether all tools were called correctly")
    tool_count_match: bool = Field(
        False, description="Whether the number of tool calls matches"
    )
    ground_truth_tool_count: int = Field(
        0, description="Number of tools in ground truth"
    )
    ...
    exact_matches: int = Field(
        0, description="Number of tools that match exactly (name and parameters)"
    )
    name_only_matches: int = Field(
        0, description="Number of tools that match by name only"
    )
    missing_tools: List[str] = Field(
        default_factory=list,
        description="Tool names in ground truth but not in prediction",
    )
    extra_tools: List[str] = Field(
        default_factory=list,
        description="Tool names in prediction but not in ground truth",
    )
    ...
    # Scoring metrics
    tool_name_score: float = Field(
        0.0, description="Percentage of tool names that match"
    )
    param_match_score: float = Field(
        0.0, description="Average parameter match score across all tools"
    )
    order_score: float = Field(
        0.0, description="How well the order of tool calls matches (0-1)"
    )
    overall_score: float = Field(0.0, description="Overall evaluation score (0-1)")

    # Match type
    match_type: str = Field(
        "strict", description="Type of matching used (strict or flexible)"
    )
...
class StaticToolEvaluator:
    ...
    def _normalize_value(value: Any) -> Any:
        ...
        if isinstance(value, str):
            ...
            return {
                StaticToolEvaluator._normalize_value(
                    k
                ): StaticToolEvaluator._normalize_value(v)
    ...
    def _compare_tool_parameters(
        ground_truth: Dict[str, Any], prediction: Dict[str, Any], flexible: bool = False
        ...
        if flexible:
            ...
            match_score = (
                partial_match_score / len(important_params) if important_params else 1.0
            )
        else:
    ...
    def evaluate_task(
        ...
        if abs(weight_sum - 1.0) > 0.001:
            w = default_weights
            print(
                f"Warning: Provided weights do not sum to 1.0. Using default weights: {default_weights}"
            )
        ...
        result.missing_tools = [
            name for name in gt_tool_names if name not in pred_tool_names
        ]
        result.extra_tools = [
            name for name in pred_tool_names if name not in gt_tool_names
        ]
        ...
        if gt_tool_names:
            name_matches = sum(
                1
                for gt_name, pred_name in zip(gt_tool_names, pred_tool_names)
        ...
        for i, (gt_call, pred_call) in enumerate(
            zip(ground_truth_calls, prediction_calls)
        ):
            if gt_call.tool_name == pred_call.tool_name:
                ...
                key = (
                    ...
                    if gt_call.tool_name
                    in [
                        call.tool_name
                        for j, call in enumerate(ground_truth_calls)
                        if j != i
                    ]
        ...
        if position_param_scores:
            result.param_match_score = sum(position_param_scores) / len(
                position_param_scores
            )
        ...
        if len(gt_tool_names) == 0 and len(pred_tool_names) == 0:
            ...
            lcs_length = StaticToolEvaluator.longest_common_subsequence(
                gt_tool_names, pred_tool_names
            )
    ...
    def evaluate_from_json(
        ...
        task_id = (
            ground_truth_json.get("task_id")
            or ground_truth_json.get("id")
            or ground_truth_json.get("name")
            or prediction_json.get("task_id")
            or "unknown"
        )

        return StaticToolEvaluator.evaluate_task(
            gt_tool_calls, pred_tool_calls, task_id, match_type
        )
    ...
    def batch_evaluate_with_both_match_types(
        ...
        for task_id in set(gt_dict.keys()).intersection(pred_dict.keys()):
            gt_tool_calls = [
                ToolCall(
                    tool_name=tc["tool_name"], tool_parameters=tc["tool_parameters"]
                )
        ...
        strict_param_scores = [
            r["strict"]["param_match_score"] for r in results.values()
        ]
        strict_order_scores = [r["strict"]["order_score"] for r in results.values()]

        flexible_name_scores = [
            r["flexible"]["tool_name_score"] for r in results.values()
        ]
        flexible_param_scores = [
            r["flexible"]["param_match_score"] for r in results.values()
        ]
        ...
        overall_stats = {
            ...
            "strict": {
                ...
                "tool_name_accuracy": (
                    np.mean(strict_name_scores) if strict_name_scores else 0.0
                ),
                "param_match_accuracy": (
                    np.mean(strict_param_scores) if strict_param_scores else 0.0
                ),
                "order_score": (
                    np.mean(strict_order_scores) if strict_order_scores else 0.0
                ),
            },
            "flexible": {
                ...
                "tool_name_accuracy": (
                    np.mean(flexible_name_scores) if flexible_name_scores else 0.0
                ),
                "param_match_accuracy": (
                    np.mean(flexible_param_scores) if flexible_param_scores else 0.0
                ),
                "order_score": (
                    np.mean(flexible_order_scores) if flexible_order_scores else 0.0
                ),
            },
        }

        return {"overall_stats": overall_stats, "task_results": results}

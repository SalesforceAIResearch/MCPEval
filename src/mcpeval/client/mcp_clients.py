...
class MCPClientStdio(BaseClient):
    def __init__(self):
        ...
        self.tool_session_mapping: Dict[str, Dict[str, Any]] = (
            {}
        )  # mapping of tool names to their respective sessions
        self.server_mapping: Dict[str, Dict[str, Any]] = (
            {}
        )  # mapping of server paths to their sessions and tools
        self.tool_name_to_session: Dict[str, ClientSession] = (
            {}
        )  # mapping of tool names to their sessions
    ...
    async def connect_to_server(
        ...
        """
        logger.info(
            f"Connecting to server {server_script_path} with args: {server_args}"
        )
        session, stdio, write = await connect_mcp_server(
            server_script_path, self.exit_stack, server_args, env
        )
    ...
    async def connect_to_multiple_servers(
        ...
        if server_envs is None:
            server_envs = [None for _ in server_script_paths]

        self.server_mapping = {}

        for server_script_path, args, env in zip(
            server_script_paths, server_args, server_envs
        ):
            session, stdio, write, tools = await self.connect_to_server(
                server_script_path, args, env
            )
        ...
        logger.info(
            f"Total tools mapped in tool_name_to_session: {len(self.tool_name_to_session)}"
        )

...
def stringify(data):
    ...
    if isinstance(data, (str, int, float, bool, type(None))):
        ...
        items = (
            stringify(key) + ": " + stringify(value) for key, value in data.items()
        )
        return "{" + ", ".join(items) + "}"
    else:
        return str(data)
...
class EnhancedJSONEncoder(json.JSONEncoder):
    def default(self, obj):
        ...
        # Handle additional types
            ...
            return self.default(
                list(obj)
            )  # Convert set to list then process recursively
        elif isinstance(obj, (list, tuple)):
            return [
                self.default(item) for item in obj
            ]  # Recursively apply to each item
            ...
            for key, value in obj.items():
                if not isinstance(key, (str, int, float, bool, type(None))):
                    key = str(
                        key
                    )  # Convert tuple keys and other non-standard types to string
    ...
    def _try_fix_json(json_str, depth=0):
        ...
        try:
            ...
            if "Extra data" in error_msg:
                extra_data_pos = int(re.findall(r"\(char (\d+)\)", error_msg)[0])
                fix_history.append(f"Removed extra data at position {extra_data_pos}")
                return _try_fix_json(json_str[:extra_data_pos], depth + 1)

            elif (
                "Expecting ',' delimiter" in error_msg
                or "Expecting property name enclosed in double quotes" in error_msg
            ):
                expecting_pos = int(re.findall(r"\(char (\d+)\)", error_msg)[0])
                if expecting_pos < len(json_str):
                    if json_str[expecting_pos] == '"':
                        ...
                        return _try_fix_json(
                            json_str[:expecting_pos] + "," + json_str[expecting_pos:],
                            depth + 1,
                        )
                ...
                expecting_pos = int(re.findall(r"\(char (\d+)\)", error_msg)[0])
                if expecting_pos < len(json_str) and json_str[expecting_pos] in [
                    ",",
                    "}",
                    "]",
                ]:
                ...
                unterminated_pos = int(re.findall(r"\(char (\d+)\)", error_msg)[0])
                fix_history.append(
                    f"Terminated unterminated string at position {unterminated_pos}"
                )
                return _try_fix_json(
                    json_str[:unterminated_pos] + '"' + json_str[unterminated_pos:],
                    depth + 1,
                )
...
def flatten_json_to_python_tool_calls(json_format):
    ...
    for call in json_format:
        for func_name, args in call.items():
            ...
            args_str = ", ".join(
                [f"{key}={repr(value)}" for key, value in args.items()]
            )
...
def python_to_api_json_tool_calls(python_format, openai_format=False):
    ...
    info (list): A list of meta information during format conversion.
    """
    flatten_json_list, info = python_to_flatten_json_tool_calls(
        python_format, openai_format
    )
    api_json = []
    for flatten_json in flatten_json_list:
        for func_name, args in flatten_json.items():
            api_json.append({"name": func_name, "arguments": args})
    ...
    for i, gt_call in enumerate(groundtruth):
        if i < len(converted) and gt_call.keys() == converted[i].keys():
            ...
            match_arguments += sum(
                1
                for arg in gt_args
                if arg in conv_args and compare_values(gt_args[arg], conv_args[arg])
            )

    match_score = (
        ((match_functions + match_arguments) / (total_functions + total_arguments))
        if (total_functions + total_arguments) > 0
        else 0
    )
    match = match_score == 1
...
def convert_python_code_to_api_json(python_functions: list):
    ...
    try:
        functions_details = [
            _extract_function_details(ast.parse(func_code).body[0])
            for func_code in python_functions
        ]

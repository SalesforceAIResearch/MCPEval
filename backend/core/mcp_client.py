...
class MCPClientManager:
    ...
    def connect_to_servers(self, servers, server_args, llm_config, server_types=None, server_envs=None):
        ...
        try:
            ...
            if server_types is None:
                server_types = ['local'] * len(servers)
            elif len(server_types) != len(servers):
                # Pad with 'local' if length mismatch
                server_types = server_types[:len(servers)]
                while len(server_types) < len(servers):
    ...
    def send_message(self, message):
        ...
        if not hasattr(self.mcp_client, 'server_mapping') or not self.mcp_client.server_mapping:
        ...
        def send_message_async():
            async def _send():
                try:
                    ...
                    if 'ClosedResourceError' in str(e) or 'connection' in str(e).lower():
                        print("MCP connection appears to be broken, invalidating client")
                        self.mcp_client = None
                    raise

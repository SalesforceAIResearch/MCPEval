...
echo "🚀 Generating comprehensive sqlite reports with sectioned charts..."
...
generate_sectioned_static_report() {
    ...
    if [ -f "benchmarks/sqlite/results/${model}_mix_task_evaluation_summary_analysis.json" ]; then
        python -c "
import sys
sys.path.insert(0, 'src')
from mcpeval.cli.analyzer.analyze import generate_sectioned_report_from_summary
generate_sectioned_report_from_summary('benchmarks/sqlite/results/${model}_mix_task_evaluation_summary_analysis.json')
"
    else
...
generate_llm_judger_report() {
    ...
    if [ -f "benchmarks/sqlite/report/${model}_mix_task_evaluation_trajectory_llm_judger_analysis.json" ]; then
        python -c "
import sys
sys.path.insert(0, 'src')
from mcpeval.cli.analyzer.analyze import generate_sectioned_report_from_summary
generate_sectioned_report_from_summary('benchmarks/sqlite/report/${model}_mix_task_evaluation_trajectory_llm_judger_analysis.json')
"
    else
...
echo "🎉 SQLite reports with sectioned charts generated successfully!"
echo ""

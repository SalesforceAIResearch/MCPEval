...
# echo "Analyzing ${MODEL} results..."
# nohup mcp-eval analyze \
#     --predictions $OUTPUT \
...
# nohup mcp-eval judge --input-file $OUTPUT --output-dir $RESULTS_DIR --model $JUDGE_MODEL > $EVAL_LOG 2>&1 &

echo "judging-rubric ${MODEL} results..."
nohup mcp-eval judge-rubric \
    --trajectory-file $TRAJECTORY_FILE \
    --completion-file $COMPLETION_FILE \
    --output-dir $REPORT_DIR --report-model $JUDGE_MODEL > $ANALYSIS_LOG 2>&1 &
